...
create table if not exists public.task_order (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  week_key date not null,
  day_key text not null check (day_key in ('sat','sun','mon','tue','wed','thu','fri')),
...
create index if not exists idx_tasks_workspace_day on public.tasks(workspace_id, day_key);
create index if not exists idx_tasks_recurring on public.tasks(workspace_id, is_recurring) where deleted_at is null;
create index if not exists idx_tasks_active on public.tasks(workspace_id, archived_at, deleted_at);
create index if not exists idx_task_instances_week on public.task_instances(workspace_id, week_key);
create index if not exists idx_task_instances_task_week on public.task_instances(task_id, week_key);
create index if not exists idx_task_notes_week on public.task_notes(workspace_id, week_key);
create index if not exists idx_task_events_week on public.task_events(workspace_id, week_key, created_at);
create index if not exists idx_alert_log_week on public.alert_log(workspace_id, week_key, sent_at);
